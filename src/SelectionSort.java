...
public class SelectionSort {
    public static void sort(ArrayList<ElectronicsInfo> sorting){
        for (int i = 0; i < sorting.size() - 1; i++){
            int minPos = minimumPosition(sorting,i);
            ElectronicsInfo temp = sorting.get(i);
            sorting.set(i,sorting.get(minPos));
            sorting.set(minPos,temp);
        }
    }
    
    public static int minimumPosition(ArrayList<ElectronicsInfo> sortArray,int from){
      int minPos = from;
